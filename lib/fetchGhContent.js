...
const createUrl = ({owner, repo, path, token}) => `https://api.github.com/repos/${owner}/${repo}/contents/${path}?${token ? 'access_token=' + token : ''}`

async function fetchGhContent ({owner, repo, path, token, isDirectory = false}) {
